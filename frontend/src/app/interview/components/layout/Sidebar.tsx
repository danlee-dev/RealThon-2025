...
export default function Sidebar({
    ...
    const menuItems = [
        { icon: PieChart, label: 'Graph', path: '/graph' },
    ...
    return (
        <div className={`bg-sidebar text-white flex flex-col transition-all duration-300 relative ${isCollapsed ? 'w-20' : 'w-60'}`}>
            ...
            <div className={`${isCollapsed ? 'px-2' : 'px-4'} mt-4`}>
                {!isCollapsed && <div className="text-xs text-gray-500 mb-3 px-3">Menu</div>}
                {menuItems.map((item) => (
                    <SidebarItem
                        key={item.label}
                        {...item}
...
function SidebarItem({ icon: Icon, label, active, isCollapsed, onClick }: { icon: any, label: string, active: boolean, isCollapsed: boolean, onClick: () => void }) {
    return (
        <button
            onClick={onClick}
